...
```{r, results='asis'}
kable(
  cluster_summary_table %>% select(-Observations),
  format = "latex",
  col.names = c("Cluster", "(Intercept)", "Food Density", "Pop Density", 
                "Avg HH Size", "Low Income", "Food Dens:Low Inc",
                "R\\^2", "F Stat"),
  ...
  kable_styling(latex_options = c("hold_position", "striped")) %>%
  landscape() %>%  # Rotate the entire table to fit better on the page
  add_header_above(c(" " = 1, "Cluster-wise Model Summary" = 8)) %>%
  footnote(general = "* p < 0.1; ** p < 0.05; *** p < 0.01", 
           general_title = "Note:", 
           footnote_as_chunk = TRUE, 
           escape = FALSE)
