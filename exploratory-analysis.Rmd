...
census_data <- get_census(dataset = "CA21",
                          regions = list(CSD = "5915022", 
                                         DA = c("59154012","59154105","59154090","59150936","59154101","59154104","59154035","59154103","59154102","59154034","59150945","59154091","59154093","59154099","59150946","59154100","59154078","59154079","59154082","59154081","59154080","59150939","59150938","59154083","59154095","59154084","59150941","59150942","59154085","59154088","59154087","59154089","59154097","59154098","59154096","59154092","59154013","59150952")),
                          vectors = vectors,
                          labels = "detailed",
                          geo_format = "sf",
                          level = "DA")
...
# some extra themes
# c1 <- crime_map + scale_color_paletteer_d("ggprism::autumn_leaves")
# c2 <- crime_map + scale_color_paletteer_d("beyonce::X70")
# c3 <- crime_map + scale_color_paletteer_d("impressionist.colors::te_aa_no_areois")
# c4 <- crime_map + scale_color_paletteer_d("ggthemes::Classic_Purple_Gray_12")


food_map <- ggplot() +
  geom_sf(data = food_data,
          color = "blue") +
  theme_minimal() +
...
combo_plot <- ggplot() +
  ...
  geom_sf(data = crime_data, aes(color = "geometry"), size = 0.4, alpha = 0.2) +
  geom_sf(data = food_data, aes(fill = "yellow"), color = "black", size = 2, alpha = 0.8, shape = 21) +
  coord_sf() + 
  labs(title = "Free and Low Cost Food Programs plotted against Crime, Vancouver") +
  theme_minimal() +
  theme(legend.position = "right") +
  scale_fill_manual(name = "", values = "yellow", labels = c("Food Programs")) +
  scale_color_manual(name = "", values = "red", labels = c("Crime")) +
